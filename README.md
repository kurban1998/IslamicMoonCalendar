...
Название города для времени намазов определяется обратным геокодированием
через **Nominatim (OpenStreetMap)** — бесплатно, без ключа. Если координаты
не переданы (геолокация недоступна), backend просто берёт `DefaultCityName`
из `appsettings.json`, не дёргая внешний сервис.

⚠️ Перед использованием в продакшене замените плейсхолдер контакта в
`Program.cs` (`UserAgent.ParseAdd(...)`) на свой e-mail/сайт — это требование
[политики использования Nominatim](https://operations.osmfoundation.org/policies/nominatim/)
(1 запрос/сек, обязательный осмысленный User-Agent). При заметной нагрузке
лучше поднять свой инстанс Nominatim или взять платный геокодер.
